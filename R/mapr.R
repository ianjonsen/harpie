...
##'
##' The telemetry data is given as a dataframe where each row is an
...
##' @param prj a PROJ.4 compatable projection for the region of interest *NOT* WGS84
##' @param buff a buffer to expand region of interest specified in metres
##' @return a list with components
##' \item{\code{world_shp}}{a projected shapefile for the region of interest}
##' @examples
##' data(ellie.ice)
##' prj <- "+proj=laea +lat_0=-60 +lon_0=70 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs"
##' world_shp <- mapr(ellie.ice, prj, buff = 1e6)
##' ggplot() + 
##'   geom_sf(aes(), data = world_shp) +
##'   geom_sf(aes(), data = st_as_sf(ellie.ice, coords = c("lon", "lat")) %>% st_set_crs("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
##'
##' @export
mapr <- function(dat,
                 prj,
